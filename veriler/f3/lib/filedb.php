...
class FileDB extends Base {
	...
	**/
	function rollback() {
		if ($glob=glob($this->path.'*-'.$this->journal))
	...
	**/
	function commit() {
		if ($glob=glob($this->path.'*-'.$this->journal))
			foreach ($glob as $temp) {
				$file=preg_replace('/-'.$this->journal.'$/','',$temp);
				rename($temp,$file);
				break;
			}
		$this->journal=NULL;
	...
	**/
	function read($file) {
		...
		if ($this->format==self::FORMAT_Plain) {
			...
			$contents=self::getfile($file);
			if ($this->format==self::FORMAT_Serialized)
				$out=unserialize($contents);
			else
	...
	**/
	function write($file,$expr) {
		...
		elseif ($this->format==self::FORMAT_Serialized)
			$out=serialize($expr);
		else
